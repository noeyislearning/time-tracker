      ...
      <div className="w-full flex flex-col text-white">
        ...
        <div className="pt-4 w-full grid grid-cols-4 gap-4 items-center text-white">
          { projects.map((project) => {
            const activeTracks = project.tracks.filter((track) => !track.end_time).length;
              ...
              >
                ...
                <div className="flex flex-row items-center gap-2">
                  <div className="flex flex-row items-center gap-1">
                    ...
                    <span className={`text-sm font-bold ${ project.tracks.length > 0 ? "text-green-500" : "text-gray-500" }`}>
                      { project.tracks.length }
                    </span>
                  </div>
                  <div className="flex flex-row items-center gap-1">
                    <span className="text-sm text-gray-500">Active:</span>
                    <span className={`text-sm font-bold ${activeTracks > 0 ? "text-green-500" : "text-gray-500"}`}>
                      {activeTracks}
                    </span>
                  </div>
                </div>
                <div className="mt-8 flex flex-row items-center gap-1">
                  ...
                  <span className="text-sm text-indigo-500 underline underline-offset-2"> {project.added_by}
                  </span>
                </div>
              </Link>
            )})}
