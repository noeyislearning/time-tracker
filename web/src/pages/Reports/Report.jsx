...
export default function Report() {
  ...
  const tracks = useSelector((state) => state.projects.selectedProject?.tracks || [], (prev, next) => prev.length === next.length)
  ...
  const countActiveTracksByDate = (groupedTracks) => { return groupedTracks.filter((track) => !track.end_time).length }
    ...
    <main className="p-7 w-full overflow-y-scroll">
      <div className="flex flex-col">
        <div className="px-3 flex flex-row items-center gap-2">
          <Link to={"/reports"} className="text-gray-500 hover:underline hover:underline-offset-2">Reports</Link>
