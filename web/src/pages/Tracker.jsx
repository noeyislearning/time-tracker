...
export default function Tracker() {
  ...
  const getTrackDate = (track) => {
    ...
    return trackDateString === currentDate ? 'Today' : trackDate.toLocaleDateString('en-US', { weekday: 'long' })
  }
  ...
  const organizeTracksByDate = () => {
    ...
    Object.keys(trackGroups).forEach((date) => {
      trackGroups[date].sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
    });
  ...
  const countActiveTracksByDate = (groupedTracks) => groupedTracks.filter((track) => !track.end_time).length
  ...
  return (
    <main className="p-7 w-full h-full overflow-y-scroll">
        ...
        { isLoading ? (
          ...
          Object.entries(trackGroups).map(([date, groupedTracks]) => {
            const activeTracks = countActiveTracksByDate(groupedTracks);
              return (
              <div key={date} className="px-3 w-full flex flex-row items-center justify-between">
                  ...
                  <div className="pt-14 w-full flex flex-row items-center justify-between">
                    <div className="flex flex-row items-center gap-4">
                      ...
                      <span className="text-gray-500">
                        { groupedTracks.length > 0 ? new Date(groupedTracks[0].start_time).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : ''}
                      </span>
                  ...
                  <div className="w-full flex flex-col gap-1">
                    {
                      groupedTracks.map((track, index) => (
                        <ProjectTrackerCard key={`${track.id}-${index}`} track={track} />
                      ))
                    }
                  </div>
                </div>
      ...
      <div className="pt-4 text-center text-gray-500">To see more, navigate to <Link to={"/reports"} className="text-indigo-600 font-bold underline underline-offset-2">Reports</Link> tab.</div>
    </main>
  )
}
