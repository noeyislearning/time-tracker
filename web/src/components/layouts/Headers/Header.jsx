...
export default function Header() {
  ...
  const weeklyTotalDuration = useSelector(
    (state) => state.tracks.weeklyTotalDuration,
  );
        ...
        <div className="flex flex-col items-end">
          <div className="text-lg flex flex-row items-center gap-2">
            ...
            {isLoading ? (
              ...
              <span className="font-bold">
                {weeklyTotalDuration
                  ? weeklyTotalDuration.total_duration_within_week
                  : "Loading..."}
              </span>
            )}
          </div>
          <div className="flex flex-row items-center gap-2">
            <span className="text-xs">
              {weeklyTotalDuration
                ? formatDate(weeklyTotalDuration.start_date)
                : "Loading..."}
            </span>
            <ArrowLongRightIcon className="w-5 h-5 text-white" />
            <span className="text-xs">
              {weeklyTotalDuration
                ? formatDate(weeklyTotalDuration.end_date)
                : "Loading..."}
            </span>
          </div>
        </div>
